  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && pathname === "/api/download") {
        ...
        if (!outDir) return badRequest(res, "Missing outDir");
        if (!ensureDirExists(outDir)) return badRequest(res, "outDir must exist and be a directory");

        const validIds = ids.filter(isValidYtId);
        if (validIds.length === 0) return badRequest(res, "No valid ids provided");
        ...
        const tmpListPath = path.join(os.tmpdir(), `ytdlp-ui-${jobId}.txt`);
        const urls = validIds.map(buildVideoUrlFromId).join("\n") + "\n";
        fs.writeFileSync(tmpListPath, urls, "utf8");

        const archivePath = path.join(outDir, ".ytdlp-archive.txt");
        ...
        try {
          ...
          jobDone(jobId, -1);
          return json(res, 200, { jobId, count: validIds.length });
        }
